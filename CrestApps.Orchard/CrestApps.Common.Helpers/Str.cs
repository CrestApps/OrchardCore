...
namespace CrestApps.Common.Helpers
{
    public class Str
    {
        ...
        {
            string template = "[%%%%% SINGLE_NEW_LINE %%%%%]";

            string body = Regex.Replace(text, "\r\n", template);
            body = Regex.Replace(body, "\r", template);
        ...
        {
            string template = Repeat(stringToReduce, reduceTo);
            string replaceWith = Repeat(stringToReduce, reduceTo - 1);
        ...
        {
            if (times < 1 || string.IsNullOrEmpty(text))
            {
                return text;
            }
        ...
        public static string SubstringUntil(string str, string until, bool trim = true, bool untilFirstOccurrence = true)
        {
            ...
            {
                int index = untilFirstOccurrence ? str.IndexOf(until) : str.LastIndexOf(until);
        ...
        public static string AfterFirstInstance(string str, string lastString)
        {
            ...
            int index = str.IndexOf(lastString);

            string substring = str.Substring(index + lastString.Length, str.Length - (index + lastString.Length));
        ...
        public static string AfterLastInstance(string str, string lastString)
        {
            ...
            int index = str.LastIndexOf(lastString);
