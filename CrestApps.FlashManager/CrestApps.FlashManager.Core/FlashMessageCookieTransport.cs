...
namespace CrestApps.FlashManager.Core
{
    /// <summary>
    ...
    public class FlashMessageCookieTransport : IFlashMessageTransport
    {
        ...
        public FlashMessageCookieTransport(IDataProtectionProvider protector)
        {

            CookieName = "_FlashMessage";
            CookieSizeLimit = 1024 * 3;
            Protector = protector.CreateProtector(nameof(FlashMessage));
        }
        ...
        public void Queue(IList<FlashMessageModel> messages)
        {
            ...
            byte[] serializedMessages = FlashMessage.Serialize(messages);
            byte[] protectedDate = Protector.Protect(serializedMessages);

            // Serialize messages and enforce cookie size limit.
            if (protectedDate.Length > CookieSizeLimit && CookieSizeLimit > 0)
            ...
            var cookie = new HttpCookie(CookieName, protectedDate);
